...
# Use relative imports since we're running the app directly
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..'))

from wheel_simulator.core.simulator import WheelSimulator
...
def display_portfolio_metrics():
    ...
    st.sidebar.markdown("### Ÿ’° Portfolio")
    ...
    if sim.portfolio.stock_positions:
        st.sidebar.markdown("### Ÿ“ˆ Stock Holdings")
...
def sell_call_interface():
    ...
    if not sim.portfolio.stock_positions:
        st.warning("¸ No stock positions available for covered calls!")
        return
...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state.initialized:
            ...
            st.markdown("---")
            if st.button("Ÿ”„ Reset Simulator"):
                st.session_state.simulator = None
                st.session_state.initialized = False
                st.rerun()
    ...
    # Main content
    if not st.session_state.initialized:
        st.info("Ÿ‘ˆ Click 'Initialize Simulator' in the sidebar to start")
        ...
        **Options Pricing:** Black-Scholes model with historical volatility
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs([
            " Market", "Ÿ“ˆ Stocks", "Ÿ“‹ Options", "… Closed", "Ÿ’¼ Trade"
        ])
        ...
        with tab2:
            st.header("Ÿ“ˆ Stock Positions")
            display_stock_positions()
        
        with tab3:
            st.header("Ÿ“‹ Open Options Positions")
        ...
        st.markdown("---")
        st.header("Ÿ“ˆ Stock Price Charts")
